...
import { Target, TrendingUp, Plus, X, CheckCircle } from 'lucide-react';
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <Textarea
                ...
                value={goals[selectedArea] || ''}
                onChange={(e) => setGoals(prev => ({...prev, [selectedArea]: e.target.value}))}
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Button 
                  onClick={() => addAction(selectedArea)} 
                  size="sm" 
                  variant="outline"
              ...
              {actions[selectedArea]?.map((action, actionIndex) => (
                <div key={actionIndex} className="flex gap-2">
                  <Input
                    placeholder="Enter an actionable step..."
                    value={action}
                    onChange={(e) => updateAction(selectedArea, actionIndex, e.target.value)}
                    className="flex-1"
                  />
                  <Button
                    onClick={() => removeAction(selectedArea, actionIndex)}
                    size="sm"
                    variant="ghost"
              ...
              {(!actions[selectedArea] || actions[selectedArea].length === 0) && (
